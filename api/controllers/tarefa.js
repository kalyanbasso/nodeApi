module.exports = app => {
    ...
    controller.getAllTask = function(req, res, next){
        ...
        'from tarefa left join projeto p on tarefa.id_projeto = p.id left join usuario on usuario.id_usuario = tarefa.id_criador ' +
        ' left join prioridade p2 on tarefa.id_prioridade = p2.id')
            .then(data => {
                res.status(200)
                    .json({
                        status: 'success',
